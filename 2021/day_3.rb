...
puts "gamma rate binary: #{gamma_rate_bin}" # if DEBUG
gamma_rate = gamma_rate_bin.to_i(2)
puts gamma_rate if DEBUG
...
def find_most_common(array, pos=0)
    puts "    ---- " if DEBUG
    puts "Finding most common in array of #{array.length}, pos #{pos}" if DEBUG
    ...
    puts "number of 0s: #{number_of_zeros}" if DEBUG
    puts "number of 1s: #{number_of_ones}" if DEBUG
...
def find_least_common(array, pos=0)
    puts "    ---- " if DEBUG
    puts "Finding least common in array of #{array.length}, pos #{pos}" if DEBUG
    puts array.inspect if DEBUG
    number_of_zeros = 0
    number_of_ones = 0
    array.each do |line|
